...
case class Actor(
  ...
  private def shortRangeCaptures(dirs: Directions, finalSquare: Boolean): List[Move] = {
    ...
    def walkCaptures(walkDir: Direction, curBoard: Board, curPos: PosMotion, destPos: Option[PosMotion], destBoard: Option[Board], allSquares: List[PosMotion], allTaken: List[PosMotion], captureValue: Int): Unit =
